...
export default function PracticePage() {
  ...
  const twoFingerScrollRef = useRef(null)
  ...
  // Two-finger touch = scroll the container under the fingers. Lets users
  // bypass single-finger drag when they need to scroll past a drill card.
  useEffect(() => {
    ...
    function onTouchStart(e) {
      ...
      const container = findScrollable(document.elementFromPoint(midX, midY))
      if (container) twoFingerScrollRef.current = { container, lastY: midY }
    }
    function onTouchMove(e) {
      if (e.touches.length !== 2 || !twoFingerScrollRef.current) return
      const midY = (e.touches[0].clientY + e.touches[1].clientY) / 2
      twoFingerScrollRef.current.container.scrollTop += twoFingerScrollRef.current.lastY - midY
      twoFingerScrollRef.current.lastY = midY
      e.preventDefault()
    }
    function onTouchEnd(e) {
      if (e.touches.length < 2) twoFingerScrollRef.current = null
    }
