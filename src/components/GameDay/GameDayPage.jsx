...
export default function GameDayPage() {
  ...
  // Printable line-up report — white background (ink-friendly), portrait,
  // sized to US Letter. Blank Goals + Qtrs columns to mark during the game.
  async function shareRosterSheet() {
    ...
    try {
      ...
      const { jsPDF } = await import('jspdf')
      // US Letter portrait: 8.5 x 11in @ 72pt = 612 x 792. Fit the canvas to the
      // page width and keep aspect ratio.
    ...
    } catch (err) { console.error('Roster sheet export error:', err) }
    finally { setIsExporting(false) }
  }
          ...
          <div style={{ background: '#1a1a2e', border: '1px solid rgba(0,200,83,0.25)', borderRadius: '16px 16px 0 0', padding: '20px 20px 32px', width: '100%', maxWidth: 420, display: 'flex', flexDirection: 'column', gap: 10 }}
            ...
            <button onClick={() => shareRosterSheet()} style={{ padding: '12px 16px', borderRadius: 10, background: 'rgba(245,200,66,0.12)', border: '1px solid rgba(245,200,66,0.35)', color: '#F5C842', fontSize: 14, fontWeight: 600, cursor: 'pointer', textAlign: 'left' }}>
              ...
              <span style={{ display: 'block', fontSize: 11, fontWeight: 400, color: 'rgba(255,255,255,0.45)', marginTop: 2 }}>
                Printable line-up report with blank quarter columns
              </span>
            </button>
